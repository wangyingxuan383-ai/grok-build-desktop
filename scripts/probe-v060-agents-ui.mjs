...
try {
  ...
  const api = await evaluate(`Promise.all([window.grokDesktop.listAgentDefinitions(${JSON.stringify(workspace)}), window.grokDesktop.listPersonaDefinitions(${JSON.stringify(workspace)})]).then(([agents, personas]) => ({ agent: agents.find((value) => value.name === 'ui-probe-agent'), persona: personas.find((value) => value.name === 'ui-probe-persona') }))`);
