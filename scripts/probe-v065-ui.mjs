...
if (!endpoint) throw new Error("Usage: node scripts/probe-v065-ui.mjs <cdp-endpoint>");
...
try {
  ...
  if (initial.version !== "0.6.5" || !initial.composer || initial.turns < 1 || initial.environmentBars) throw new Error(`0.6.5 shell mismatch: ${JSON.stringify(initial)}`);
  ...
  console.log(JSON.stringify({ ok: true, version: initial.version, navigation: "dashboard→chat→file→chat→tasks→chat", rightTools: launcher.length, recentFilePreview: true, nonGitReview: "empty-state", structuredError: "collapsed-with-details", turnMetrics: true, updateActions: 4, diagnosticsNavigation: true, responsiveComposer: true, narrowDrawerVisible: true, providerManager: true }, null, 2));
