...
try {
  ...
  const appVersion = await evaluate("window.grokDesktop.bootstrap().then(value => value.appVersion)");
  if (appVersion !== "0.6.1") throw new Error(`Main process reported ${appVersion}`);
  await evaluate("document.querySelector('.sidebar-footer button[title=\"版本与更新\"]')?.click(); true");
  await waitFor(() => evaluate("document.querySelector('.control-panel .about')?.innerText.includes('Grok Build Desktop 0.6.1')"), "About panel did not report 0.6.1");
  ...
  const diagnostics = await evaluate(`({
    heading: document.querySelector('.diagnostics-panel h2')?.textContent || '',
    overall: document.querySelector('.diagnostic-overall')?.textContent || '',
    supportPreview: document.querySelector('.support-preview')?.innerText || '',
  })`);
  ...
  console.log(JSON.stringify({ ok: true, mainProcessVersion: appVersion, aboutVersion: "0.6.1", diagnostics: diagnostics.overall.trim(), supportBundleAttachmentExclusion: true }, null, 2));
} finally {
  socket.close();
}
