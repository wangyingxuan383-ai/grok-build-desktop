...
try {
  ...
  if (bootstrap.appVersion !== "0.6.2") throw new Error(`Main process reported ${bootstrap.appVersion}`);
  await evaluate("document.querySelector('.sidebar-footer button[title=\"版本与更新\"]')?.click(); true");
  await waitFor(() => evaluate("document.querySelector('.control-panel .about')?.innerText.includes('Grok Build Desktop 0.6.2')"), "About panel did not report 0.6.2");
  ...
  console.log(JSON.stringify({ ok: true, mainProcessVersion: bootstrap.appVersion, aboutVersion: "0.6.2", channel: bootstrap.channel, diagnostics: diagnostics.overall.trim(), supportBundleAttachmentExclusion: true }, null, 2));
