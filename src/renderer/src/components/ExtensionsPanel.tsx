...
function ComputerTab({ setError }: { setError(message: string): void }): React.JSX.Element {
  ...
  return <TabShell title="Grok Computer Use（实验性）" action={<button onClick={load}>重新诊断</button>}>
    {!capability || !settings ? <Loading /> : <><div className={`capability-card ${capability.available ? "ok" : "failed"}`}><strong>{capability.available ? "Windows Harness 已就绪" : "Computer Use 暂不可用"}</strong><p>{capability.available ? `Helper ${capability.helperVersion} · loopback MCP · PNG 图片结果 · /computer Skill` : capability.diagnostics.join("；")}</p></div>
      <div className="tab-note"><strong>{capability.evidence?.liveVerified ? "当前环境已实测" : "当前环境尚未完成模型实测"}：</strong>{capability.acceptanceSummary}<p>组件配置：{capability.evidence?.configured ? "已开启" : "未开启"} · 会话注入：{capability.evidence?.injected ? "已有连接" : "未观察到"} · 调用请求：{capability.evidence?.requested ? "已收到" : "未收到"} · 身份校验：{capability.evidence?.discovered ? "已有通过记录" : "尚未通过"}</p></div>
      <label className="computer-toggle"><input type="checkbox" checked={settings.enabled} disabled={!capability.available} onChange={(event) => void patch({ enabled: event.target.checked })} /><span><strong>允许 Computer Use</strong><small>开启后允许模型按任务选择电脑工具；未使用时保持休眠。</small></span></label>
