...
function DefinitionFields({ kind, definition, buffer, disabled, onChange }: { kind: DefinitionKind; definition: Definition; buffer: string; disabled: boolean; onChange(value: string): void }): React.JSX.Element {
  ...
  return <details className="definition-fields" open><summary>结构化字段</summary><div className="definition-field-grid"><label>说明<input disabled={disabled} defaultValue={value.description ?? ""} onBlur={(event) => field("description", event.target.value || undefined)}/></label><label>指令文件<input disabled={disabled} defaultValue={value.instructionFile ?? ""} onBlur={(event) => field("instructions_file", event.target.value || undefined)}/></label><label>模型<input disabled={disabled} defaultValue={value.modelId ?? ""} onBlur={(event) => field("model", event.target.value || undefined)}/></label><label>推理强度<select disabled={disabled} defaultValue={value.effort ?? ""} onChange={(event) => field("reasoning_effort", event.target.value || undefined)}><option value="">继承</option>{["auto", "none", "minimal", "low", "medium", "high", "xhigh", "max"].map((item) => <option key={item}>{item}</option>)}</select></label><label>默认能力<input disabled={disabled} defaultValue={value.defaultCapabilityMode ?? ""} placeholder="all / read-only" onBlur={(event) => field("default_capability_mode", event.target.value || undefined)}/></label><label>默认隔离<select disabled={disabled} defaultValue={value.defaultIsolation ?? "none"} onChange={(event) => field("default_isolation", event.target.value)}><option value="none">none</option><option value="worktree">worktree</option></select></label><label className="definition-check"><input type="checkbox" disabled={disabled} defaultChecked={value.defaultForkContext ?? false} onChange={(event) => field("default_fork_context", event.target.checked)}/>默认继承上下文</label><div className="definition-contracts"><span>Inputs {value.inputContract.length}</span><span>Outputs {value.outputContract.length}</span><small>契约数组请在下方原始 TOML 中编辑</small></div></div></details>;
}
