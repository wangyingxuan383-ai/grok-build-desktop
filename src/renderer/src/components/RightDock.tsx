...
import type { NavigationIntent, PromptQueueEntry } from "../../../shared/types";
 ...
 */
export function RightDock({ tool, active, sessionId, cwd, lastTurnPaths, reviewInitialScope, turn, queue, sessionStatus, onTool, onClose, onNavigate, onAddComment, onExpandResult, onError }: {
  tool: RightTool | null;
  active: boolean;
  sessionId: string;
  cwd: string;
  ...
  return <Suspense fallback={<aside className="right-utility-pane workbench-loading" role="status"><div className="spinner"/><span>正在加载侧栏…</span></aside>}>
    {tool === "agent-changes" ? <AgentChangePane key={paneKey} sessionId={sessionId} onClose={onClose} onNavigate={onNavigate} onError={onError}/>
      : tool === "review" ? <ReviewPane key={paneKey} cwd={cwd} sessionId={sessionId} lastTurnPaths={lastTurnPaths} initialKind={reviewInitialScope} onClose={onClose} onNavigate={onNavigate} onAddComment={onAddComment} onError={onError}/>
        : <RightUtilityPane key={paneKey} tool={tool} turn={turn} cwd={cwd} sessionId={sessionId} paths={lastTurnPaths} queue={queue} sessionStatus={sessionStatus} onTool={onTool} onClose={onClose} onNavigate={onNavigate} onExpandResult={onExpandResult} onError={onError}/>} 
  </Suspense>;
}
