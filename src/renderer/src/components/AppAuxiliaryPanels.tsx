...
export function SessionHistoryPanel({ sessionId, onClose, onForked, onRewound, confirmAction }: { sessionId: string; onClose(): void; onForked(result: { sessionId: string; parentSessionId: string; cwd: string }): void; onRewound(): void; confirmAction(message: string, options?: { title?: string; confirmLabel?: string; danger?: boolean }): Promise<boolean> }): React.JSX.Element {
  ...
  const rewind = async (point: RewindPoint): Promise<void> => { if (!await confirmAction(`将对话历史回退到“${point.label}”？工作区文件不会改变；需要恢复文件时请使用 Review。`, { title: "回退对话", confirmLabel: "回退对话" })) return; setBusy(true); setNotice(""); try { await window.grokDesktop.rewindSession(sessionId, point.id); onRewound(); } catch (error) { setNotice(`当前 CLI 无法回退：${errorMessage(error)}`); } finally { setBusy(false); } };
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="control-panel session-history-panel" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><header><div><h2>分叉与回退</h2><small>CLI 1.0.1+ 的回退只截断对话历史，不恢复工作区文件。</small></div><button onClick={onClose}>×</button></header><div className="panel-scroll"><div className="fork-profile-picker"><label>分叉执行配置档<select value={profileId} onChange={(event) => setProfileId(event.target.value)}>{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.name}</option>)}</select></label>{selectedProfile?.worktree && <><label>Worktree 名称<input value={worktreeName} onChange={(event) => setWorktreeName(event.target.value)} placeholder="留空自动生成"/></label><label>基础 Ref<input value={worktreeRef} onChange={(event) => setWorktreeRef(event.target.value)} placeholder={selectedProfile.worktreeRef || "当前 HEAD"}/></label></>}</div><button className="primary" disabled={busy || !profileId} onClick={() => void fork()}>从当前末尾创建分叉</button>{notice && <p className="inline-error" role="status">{notice}</p>}{loading ? <p>正在读取可用回退点…</p> : points.length ? <div className="rewind-list">{points.map((point) => <article key={point.id}><div><strong>{point.label}</strong><span>{point.userMessage || point.createdAt || point.id}</span><small>{point.filesChanged !== undefined ? `此点曾涉及 ${point.filesChanged} 个文件；CLI 回退不会修改它们` : "文件恢复请使用 Review"}</small></div><div className="provider-actions"><button disabled={busy || !profileId} onClick={() => void fork(point.id)}>从这里分叉</button><button disabled={busy} onClick={() => void rewind(point)}>回退对话</button></div></article>)}</div> : <p className="empty-copy">当前 CLI 没有提供回退点，或此会话尚无可回退内容。</p>}</div></section></div>;
}

export function ControlPanel({ type, onClose, confirmAction, onDiagnostics, onProviders, onOnboarding }: { type: "settings" | "accounts" | "about"; onClose(): void; onDiagnostics(): void; onProviders(): void; onOnboarding(): void; confirmAction(message: string, options?: { title?: string; confirmLabel?: string; danger?: boolean }): Promise<boolean> }): React.JSX.Element {
  ...
  const [cliCheckMessage, setCliCheckMessage] = useState("");
  ...
  const checkCliNow = async (): Promise<void> => {
    if (cliChecking) return;
    setCliChecking(true);
    setCliCheckMessage("正在检查 Grok CLI stable 通道…");
    try {
      const value = await window.grokDesktop.checkCliUpdate();
      store.setCli(value);
...
export function OfficialFeedbackDialog({ sessionId, onClose }: { sessionId: string; onClose(): void }): React.JSX.Element {
  const [capability, setCapability] = useState<Awaited<ReturnType<typeof window.grokDesktop.getOfficialFeedbackCapability>>>();
  const [text, setText] = useState("");
  const [preview, setPreview] = useState<Awaited<ReturnType<typeof window.grokDesktop.previewOfficialFeedback>>>();
  const [busy, setBusy] = useState(false);
  const [notice, setNotice] = useState("");
  useEffect(() => {
    setCapability(undefined); setPreview(undefined); setNotice("");
    if (!sessionId) { setCapability({ available: false, source: "unavailable", reason: "请先打开一个 Grok 会话。" }); return; }
    void window.grokDesktop.getOfficialFeedbackCapability(sessionId).then(setCapability).catch((error) => setNotice(errorMessage(error)));
  }, [sessionId]);
  const buildPreview = async (): Promise<void> => {
    setBusy(true); setNotice("");
    try { setPreview(await window.grokDesktop.previewOfficialFeedback(text)); }
  ...
  const submit = async (): Promise<void> => {
    ...
    setBusy(true); setNotice("");
    try {
      const receipt = await window.grokDesktop.submitOfficialFeedback(sessionId, text);
      setNotice(receipt.message); setText(""); setPreview(undefined);
    } catch (error) { setNotice(errorMessage(error)); }
    finally { setBusy(false); }
  };
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="control-panel official-feedback-panel" role="dialog" aria-modal="true" aria-labelledby="official-feedback-title" onMouseDown={(event) => event.stopPropagation()}><header><div><h2 id="official-feedback-title">Grok Build 官方反馈</h2><small>只在当前 CLI 会话真实发布 /feedback 时启用。</small></div><button aria-label="关闭" onClick={onClose}>×</button></header><div className="panel-body">{!capability ? <p>正在检查当前会话的官方反馈能力…</p> : !capability.available ? <div className="empty-copy"><strong>当前不可用</strong><p>{capability.reason}</p><p>Desktop 自身问题仍可通过帮助菜单中的 GitHub Issues 反馈。</p></div> : <><label className="field"><span>反馈内容</span><textarea rows={8} maxLength={65_536} value={text} onChange={(event) => { setText(event.target.value); setPreview(undefined); setNotice(""); }} placeholder="描述 Grok Build CLI 的问题或建议。不要填写密码、API Key 或私人路径。"/></label><small>{text.length.toLocaleString()} / 65,536 字符</small>{preview && <section className="feedback-preview"><header><strong>即将提交的脱敏预览</strong>{preview.redacted && <span>已移除疑似凭据、邮箱、URL 参数或本地路径</span>}</header><pre>{preview.preview}</pre></section>}<div className="button-row"><button disabled={busy || !text.trim()} onClick={() => void buildPreview()}>生成脱敏预览</button><button className="primary" disabled={busy || !preview} onClick={() => void submit()}>{busy ? "处理中…" : "确认并提交"}</button></div></>}{notice && <p className="settings-note" role="status" aria-live="polite">{notice}</p>}</div></section></div>;
}
...
function SettingsDialog({ initial, knownModels, onClose, onDiagnostics, onProviders }: { initial: AppSettings; knownModels: Array<{ modelId: string; name: string }>; onClose(): void; onDiagnostics(): void; onProviders(): void }): React.JSX.Element {
  ...
  const [hiddenWorkspaces, setHiddenWorkspaces] = useState<WorkspaceSummary[]>([]);
  ...
  const [catalog, setCatalog] = useState<ModelInfo[]>(knownModels);
  useEffect(() => {
    void window.grokDesktop.listModelCatalog().then((models) => {
      if (models.length) setCatalog(models);
    }).catch((error) => store.setError(errorMessage(error)));
  }, [store]);
  const models = catalog.length ? catalog : knownModels;
  const effortControl = effortControlState(models, draft.defaultModel || models[0]?.modelId, draft.defaultEffort);
  ...
  useEffect(() => { if (category === "project") void window.grokDesktop.listHiddenWorkspaces().then(setHiddenWorkspaces).catch((error) => store.setError(errorMessage(error))); }, [category]);
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="settings-dialog" role="dialog" aria-modal="true" aria-label="设置" onMouseDown={(event) => event.stopPropagation()}>
    ...
    <div className="settings-layout"><nav aria-label="设置分类">{categories.map((item) => <button key={item.id} className={category === item.id ? "active" : ""} onClick={() => setCategory(item.id)}><UiIcon name={item.icon}/><span>{item.label}</span></button>)}</nav><main className="settings-content">
      {category === "general" && <SettingsSection title="常规" description="调整整个桌面应用的阅读密度和本地 CLI 位置。"><label>Grok CLI 路径<input value={draft.cliPath} placeholder="自动发现 %USERPROFILE%\\.grok\\bin\\grok.exe" onChange={(event) => setDraft({ ...draft, cliPath: event.target.value })}/></label><label>文字大小 <strong>{draft.fontScale}%</strong><input type="range" min="85" max="130" step="5" value={draft.fontScale} onChange={(event) => setDraft({ ...draft, fontScale: Number(event.target.value) })}/></label><label>界面密度<select value={draft.uiDensity} onChange={(event) => setDraft({ ...draft, uiDensity: event.target.value as AppSettings["uiDensity"] })}><option value="compact">紧凑</option><option value="balanced">标准（推荐）</option><option value="comfortable">宽松</option></select></label><fieldset className="conversation-reading-settings"><legend>对话阅读</legend><label>正文宽度 <strong>{draft.conversationContentWidth ?? 780}px</strong><input type="range" min="640" max="1040" step="20" value={draft.conversationContentWidth ?? 780} onChange={(event) => setDraft({ ...draft, conversationContentWidth: Number(event.target.value) })}/><small>只影响消息、过程与最终回答，不改变 Review 或编辑器。</small></label><label>正文字号 <strong>{draft.conversationFontScale ?? 100}%</strong><input type="range" min="90" max="135" step="5" value={draft.conversationFontScale ?? 100} onChange={(event) => setDraft({ ...draft, conversationFontScale: Number(event.target.value) })}/><small>只调整对话正文；侧栏、标题栏和工作台保持原尺寸。</small></label></fieldset></SettingsSection>}
      ...
      {category === "models" && <SettingsSection title="模型与会话" description="默认值用于新任务；列表与会话输入框使用同一套 CLI/ACP 模型目录和该模型声明的思考档位。"><label>默认模型<select value={draft.defaultModel} onChange={(event) => {
        const modelId = event.target.value;
        const nextEffort = effortControlState(models, modelId, draft.defaultEffort);
        setDraft({
          ...draft,
          defaultModel: modelId,
          defaultEffort: nextEffort.supported && draft.defaultEffort && !nextEffort.options.some((item) => item.value === draft.defaultEffort) ? "" : draft.defaultEffort,
        });
      }}><option value="">CLI 默认</option>{draft.defaultModel && !models.some((model) => model.modelId === draft.defaultModel) && <option value={draft.defaultModel}>{draft.defaultModel}（当前）</option>}{models.map((model) => <option key={model.modelId} value={model.modelId}>{model.name}</option>)}</select></label><label>默认推理强度<select value={draft.defaultEffort} disabled={!effortControl.supported} title={effortControl.reason} onChange={(event) => setDraft({ ...draft, defaultEffort: event.target.value as ReasoningEffort })}><option value="">CLI 默认</option>{effortControl.options.map((item) => <option key={item.value} value={item.value} title={item.description}>{item.label}</option>)}</select></label><label>默认模式<select value={draft.defaultMode} onChange={(event) => setDraft({ ...draft, defaultMode: event.target.value as SessionMode })}><option value="agent">Agent（询问权限）</option><option value="plan">Plan（自动批准，可编辑）</option><option value="auto">自动批准</option></select></label>{!models.length && <p className="settings-note">还没有模型目录。打开任一会话后会自动填充官方模型和思考档位。</p>}<label className="check"><input type="checkbox" checked={draft.showThinking} onChange={(event) => setDraft({ ...draft, showThinking: event.target.checked })}/>显示完整思考过程</label><label className="check"><input type="checkbox" checked={draft.expandToolDetails} onChange={(event) => setDraft({ ...draft, expandToolDetails: event.target.checked })}/>默认展开工具详情和 Diff</label></SettingsSection>}
      {category === "project" && <SettingsSection title="项目与 Git" description="Review 使用真实会话/Worktree 执行目录，并在主进程校验仓库边界。"><label>HTTP 代理<input value={draft.httpProxy} onChange={(event) => setDraft({ ...draft, httpProxy: event.target.value })}/></label><label>HTTPS 代理<input value={draft.httpsProxy} onChange={(event) => setDraft({ ...draft, httpsProxy: event.target.value })}/></label><dl className="settings-facts"><dt>当前项目</dt><dd>{draft.activeWorkspace || "未选择"}</dd><dt>Git 写操作</dt><dd>固定参数数组与 stdin，不经过 shell</dd></dl><div className="hidden-workspace-settings"><header><strong>已隐藏项目</strong><span>{hiddenWorkspaces.length} 个</span></header>{hiddenWorkspaces.length ? hiddenWorkspaces.map((workspace) => <article key={workspace.projectId}><span><strong>{workspace.name}</strong><small title={workspace.displayPath}>{workspace.displayPath}</small>{!workspace.exists && <em>{workspace.diagnostic || "路径已失效"}</em>}</span><button onClick={() => void window.grokDesktop.setWorkspaceHidden(workspace.cwd, false).then(async (rows) => { store.setWorkspaces(rows); setHiddenWorkspaces(await window.grokDesktop.listHiddenWorkspaces()); }).catch((error) => store.setError(errorMessage(error)))}>恢复</button></article>) : <p className="settings-note">没有隐藏项目。隐藏只影响目录展示，不删除会话或源文件。</p>}</div></SettingsSection>}
...
function QuotaPanel({ quota, loading, onRefresh }: { quota: GrokQuotaSnapshot | null; loading: boolean; onRefresh(): void }): React.JSX.Element {
  return <section className="quota-panel"><header><div><h3>账号额度</h3>{quota && <small>{quota.stale ? "缓存数据" : "更新于"} {new Date(quota.fetchedAt).toLocaleString()}</small>}</div><button disabled={loading} onClick={onRefresh}>{loading ? "刷新中…" : "刷新"}</button></header>{!quota ? <p>正在查询额度…</p> : !quota.supported ? <p>{quota.diagnostics[0]}</p> : <div className="quota-grid">{quota.rolling24h && <QuotaCard value={quota.rolling24h} />}{quota.currentAllowance && <QuotaCard value={quota.currentAllowance} />}{!quota.currentAllowance && quota.weekly && <QuotaCard value={quota.weekly} />}{!quota.currentAllowance && quota.monthly && <QuotaCard value={quota.monthly} />}{(quota.payAsYouGo ?? quota.onDemand) && <QuotaCard value={(quota.payAsYouGo ?? quota.onDemand)!} />}{(quota.prepaidBalance ?? 0) > 0 && quota.autoTopupRule && <div className="quota-card"><strong>自动充值</strong><div><b>{quota.autoTopupRule.enabled ? "已启用" : "未启用"}</b></div>{quota.autoTopupRule.enabled && quota.autoTopupRule.topupAmount !== undefined && <small>每次充值：{quotaAmount(quota.autoTopupRule.topupAmount, "credits")}</small>}{quota.autoTopupRule.enabled && quota.autoTopupRule.minBeforeLimit !== undefined && <small>触发余额：{quotaAmount(quota.autoTopupRule.minBeforeLimit, "credits")}</small>}{quota.autoTopupRule.enabled && quota.autoTopupRule.monthlyCap !== undefined && <small>每月上限：{quotaAmount(quota.autoTopupRule.monthlyCap, "credits")}</small>}</div>}{(quota.prepaidBalance ?? 0) > 0 && <div className="quota-card"><strong>已购额度余额</strong><div><b>{quotaAmount(quota.prepaidBalance, "credits")}</b></div></div>}{quota.subscriptionTier && <div className="quota-card"><strong>订阅方案</strong><div><b>{quota.subscriptionTier}</b></div></div>}{quota.currentAllowance?.periodType === "weekly" && <div className="quota-diagnostics"><p>官方 credits 接口当前只返回周订阅周期，没有返回可独立展示的月订阅额度；应用不会用旧字段估算。按量付费月上限仅在账号启用且接口返回正数时单独显示。</p></div>}{quota.currentAllowance?.periodType === "monthly" && <div className="quota-diagnostics"><p>官方 credits 接口当前返回月订阅周期；没有额外的周订阅额度，应用不会重复生成。</p></div>}{quota.evidence?.length ? <div className="quota-diagnostics">{quota.evidence.map((value) => <p key={`${value.source}:${value.observedAt}`}>数据证据：{value.source === "cli-extension" ? "CLI x.ai/billing" : value.source === "credits-api" ? "billing?format=credits" : value.source} · {value.periodType ?? "周期未知"}</p>)}</div> : null}{quota.diagnostics.length > 0 && <div className="quota-diagnostics">{quota.diagnostics.map((value) => <p key={value}>{value}</p>)}</div>}</div>}</section>;
}
