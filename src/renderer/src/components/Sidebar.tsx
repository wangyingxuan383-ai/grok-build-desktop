...
import { groupSessionsByOrigin } from "../session-groups";
...
export function Sidebar(props: {
  ...
  return <aside className="sidebar">
    ...
    <div className="session-list">
      ...
      {Boolean(activeWorkspace?.draftCount) && <div className={`session-row draft ${props.activeSessionId ? "" : "active"}`}><button type="button" className="session-open" onClick={props.onNew} aria-current={!props.activeSessionId ? "page" : undefined}><span className="status-dot cold"/><span className="session-copy"><strong>未发送草稿</strong><span>尚未启动 Grok CLI</span></span></button><button type="button" className="session-row-dismiss" title="删除草稿" aria-label="删除未发送草稿" onClick={props.onDeleteDraft}><UiIcon name="trash" size={14}/></button></div>}
      {sessionGroups.filter((group) => group.kind === "normal" || group.sessions.length > 0).map((group) => { const collapsed = props.settings?.sessionGroupCollapsed?.[group.kind] ?? group.kind !== "normal"; return <div className={`session-origin-group ${group.kind}`} key={group.kind}><button className="session-group-heading" onClick={() => props.onToggleSessionGroup(group.kind, !collapsed)}><strong>{collapsed ? "›" : "⌄"} {group.label}</strong><span>{group.sessions.length}</span></button>{!collapsed && group.sessions.map((session) => <SessionListRow key={session.id} session={session} active={props.activeSessionId === session.id} menuOpen={openSessionMenu === session.id} onOpen={() => props.onOpen(session)} onMenu={(open) => setOpenSessionMenu((current) => open ? session.id : current === session.id ? "" : current)} onPin={() => props.onPin(session)} onArchive={() => props.onArchive(session)} onExport={() => props.onExport(session)} onRename={() => props.onRename(session)} onDelete={() => props.onDelete(session)} />)}</div>; })}
      ...
      {!props.settings?.codexGroupCollapsed && <><label className="archived-toggle"><input type="checkbox" checked={props.settings?.showArchivedCodex ?? false} onChange={(event) => props.onToggleArchived(event.target.checked)} />显示归档</label>{props.codexSessions.map((session) => <div key={session.id} className={`session-row external codex ${props.activeCodexId === session.id ? "active" : ""}`}><button className="session-open" onClick={() => props.onOpenCodex(session)}><span className="codex-mark">C</span><span className="session-copy"><strong>{session.title}</strong><span>{relativeTime(session.updatedAt)}{session.archived ? " · 已归档" : ""}</span></span></button><button className="session-row-dismiss" title="从镜像列表隐藏" aria-label={`隐藏 ${session.title}`} onClick={() => props.onHideCodex(session)}>×</button></div>)}</>}
      ...
      {!props.settings?.claudeGroupCollapsed && props.claudeSessions.map((session) => <div key={session.id} className={`session-row external claude ${props.activeClaudeId === session.id ? "active" : ""}`}><button className="session-open" onClick={() => props.onOpenClaude(session)}><span className="claude-mark">A</span><span className="session-copy"><strong>{session.title}</strong><span>{relativeTime(session.updatedAt)}{session.model ? ` · ${session.model}` : ""}</span></span></button><button className="session-row-dismiss" title="从镜像列表隐藏" aria-label={`隐藏 ${session.title}`} onClick={() => props.onHideClaude(session)}>×</button></div>)}
