...
export function Composer(props: {
  ...
  /** Plan approval is an ACP waiting state, but official Grok permits model
   * changes before the decision is submitted. Keep other waiting states locked. */
  ...
  onDraftChange?(patch: Partial<NewTaskDraft>): void;
  onSend(): void;
  onInterject(): void;
  /** Non-blocking x.ai/btw side question advertised by the current CLI. */
...
function DraftModelControls({ draft, models, disabled, onChange }: { draft: NewTaskDraft; models: ModelInfo[]; disabled: boolean; onChange(patch: Partial<NewTaskDraft>): void }): React.JSX.Element {
  ...
  return <div className="model-controls draft-model-controls" aria-label="新任务草稿配置">
    <select aria-label="草稿模型" className="model-select" disabled={disabled} value={draft.modelId ?? ""} onChange={(event) => onChange({ modelId: event.target.value || undefined })}><option value="">桌面默认模型</option>{values.map((model) => <option key={model.modelId} value={model.modelId}>{model.name}</option>)}</select>
    <select aria-label="草稿推理强度" className="effort-select" disabled={disabled} value={draft.effort ?? ""} onChange={(event) => onChange({ effort: event.target.value as ReasoningEffort })}><option value="">CLI 默认</option>{["auto", "none", "minimal", "low", "medium", "high", "xhigh", "max"].map((value) => <option key={value} value={value}>{value}</option>)}</select>
