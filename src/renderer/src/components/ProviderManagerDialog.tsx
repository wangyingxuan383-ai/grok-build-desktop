...
import type { CustomProviderInput, CustomProviderProfile, ProviderConnectionDraft, ProviderDraftProbeResult, ProviderModelCandidate, ProviderModelDefinition, ProviderProtocol, ProviderProxyMode, ProviderSchemaProfile, ProviderUpstreamProtocol } from "../../../shared/types";
...
function ProviderDraftEditor({ draft, setDraft, candidates, setCandidates, visibleCandidates, selectedCandidates, setSelectedCandidates, modelSearch, setModelSearch, probe, busy, onProbe, onDiscover, onImport, onSave, onCancel }: {
  ...
  return <div className="provider-draft-editor"><header><div><strong>{draft.models?.length ? "编辑提供商" : "添加提供商"}</strong><span>模型发现仅访问列表端点，不发送推理请求。</span></div><div><button onClick={onCancel}>取消</button><button className="primary" disabled={Boolean(busy) || !(draft.models?.length)} onClick={onSave}>{busy === "save" ? "保存中…" : "保存"}</button></div></header><div className="provider-draft-scroll">
    ...
    <section><div className="provider-model-heading"><div><h3>将保存的模型</h3><span>每个模型可独立选择协议、空闲超时与思考档位</span></div><button onClick={() => setDraft({ ...draft, models: [...(draft.models ?? []), { id: `${draft.id}-model`, model: "", name: "", contextWindow: undefined, inferenceIdleTimeoutSeconds: DEFAULT_PROVIDER_INFERENCE_IDLE_TIMEOUT_SECONDS }] })}>+ 手工添加</button></div><div className="provider-imported-models">{(draft.models ?? []).map((model, index) => <div key={`${index}:${model.id}`}><input value={model.id} placeholder="本地配置 ID" onChange={(event) => updateModel(index, { id: event.target.value })}/><input value={model.model} placeholder="远端模型 ID" onChange={(event) => updateModel(index, { model: event.target.value, reasoningEfforts: providerReasoningEfforts(event.target.value, model.reasoningEfforts) })}/><input value={model.name} placeholder="显示名称" onChange={(event) => updateModel(index, { name: event.target.value })}/><input type="number" min="1024" value={model.contextWindow ?? ""} placeholder="上下文未知" onChange={(event) => updateModel(index, { contextWindow: event.target.value ? Number(event.target.value) : undefined })}/><button onClick={() => setDraft({ ...draft, models: (draft.models ?? []).filter((_value, position) => position !== index) })}>删除</button><div className="provider-model-options"><label>请求协议<select value={model.protocol ?? ""} onChange={(event) => updateModel(index, { protocol: event.target.value ? event.target.value as ProviderProtocol : undefined })}><option value="">继承 Provider（{protocolLabel(draft.protocol)}）</option><option value="chat_completions">Chat Completions</option><option value="responses">Responses</option><option value="messages">Anthropic Messages</option></select></label><label>推理空闲超时<input type="number" min="30" max="3600" step="30" value={model.inferenceIdleTimeoutSeconds ?? DEFAULT_PROVIDER_INFERENCE_IDLE_TIMEOUT_SECONDS} onChange={(event) => updateModel(index, { inferenceIdleTimeoutSeconds: Number(event.target.value) })}/><span>秒</span></label><div className="provider-reasoning-efforts"><span>思考程度</span>{PROVIDER_REASONING_EFFORTS.map((effort) => <label key={effort}><input type="checkbox" checked={model.reasoningEfforts?.includes(effort) ?? false} onChange={(event) => { const current = model.reasoningEfforts ?? []; updateModel(index, { reasoningEfforts: event.target.checked ? [...current, effort] : current.filter((value) => value !== effort) }); }}/>{effort}</label>)}</div></div></div>)}</div><p className="provider-probe-warning">推理空闲超时只计算连续未收到流事件的时间，默认 600 秒（与当前 Grok CLI 原生默认一致）；整个工具回合仍可继续更久。思考档位由上游元数据或你的配置决定。</p></section>
  </div></div>;
}
...
function ProviderDetails({ provider, busy, probe, onEdit, onTest, onRemove, onDesktopDefault, onCliDefault }: { provider: CustomProviderProfile; busy: string; probe?: ProviderDraftProbeResult; onEdit(): void; onTest(): void; onRemove(): void; onDesktopDefault(id: string): void; onCliDefault(id: string): void }): React.JSX.Element {
  return <div className="provider-details"><header><div><strong>{provider.name}</strong><span>{provider.id} · {protocolLabel(provider.protocol)}</span></div><div><button disabled={Boolean(busy)} onClick={onTest}>测试</button>{provider.owned && <button disabled={Boolean(busy)} onClick={onEdit}>编辑</button>}{provider.owned && <button className="danger-text" disabled={Boolean(busy)} onClick={onRemove}>删除</button>}</div></header><dl><dt>基础地址</dt><dd>{provider.baseUrl}</dd><dt>模型列表</dt><dd>{provider.modelListUrl || `${provider.baseUrl}/models`}</dd><dt>网络路由</dt><dd>{provider.proxyMode === "direct" ? "直连（跳过应用代理）" : "继承应用代理"}</dd><dt>认证</dt><dd>{provider.credentialMode === "none" ? "无需认证" : provider.hasCredential ? "已配置" : "缺少凭据"}</dd><dt>来源</dt><dd>{provider.owned ? "Grok Build Desktop 管理" : "外部 config.toml（只读）"}</dd></dl>{probe && <p className={probe.ok ? "provider-result success" : "provider-result error"}>{probe.message} · {probe.latencyMs} ms</p>}<section><h3>模型 <span>{provider.models.length}</span></h3>{provider.models.map((model) => <article key={model.id}><div><strong>{model.name}</strong><small>{model.id} → {model.model}</small><span>{protocolLabel(model.protocol ?? provider.protocol)} · 空闲超时 {model.inferenceIdleTimeoutSeconds ?? DEFAULT_PROVIDER_INFERENCE_IDLE_TIMEOUT_SECONDS} 秒 · {model.contextWindow ? `${Math.round(model.contextWindow / 1000)}K 上下文` : "上下文未知 / 服务默认"} · {model.reasoningEfforts?.length ? `思考：${model.reasoningEfforts.join(" / ")}` : "未声明思考档位"}</span></div><button onClick={() => onDesktopDefault(model.id)}>桌面默认</button><button onClick={() => onCliDefault(model.id)}>CLI 默认</button></article>)}</section></div>;
}

function presetDraft(preset: Preset): ProviderConnectionDraft {
  const base: ProviderConnectionDraft = { id: "custom", name: "自定义提供商", baseUrl: "https://api.example.com/v1", protocol: "chat_completions", upstreamProtocol: "openai_chat", schemaProfile: "standard", proxyMode: "inherit", authScheme: "bearer", credentialMode: "managed", credentialValue: "", headers: [], models: [] };
  ...
  if (preset === "gemini-compatible") return { ...base, id: "gemini-compatible", name: "Gemini 兼容网关", baseUrl: "https://api.example.com/v1", schemaProfile: "gemini", upstreamProtocol: "gemini_generate_content" };
...
function profileDraft(profile: CustomProviderProfile): ProviderConnectionDraft { return { id: profile.id, name: profile.name, baseUrl: profile.baseUrl, modelListUrl: profile.modelListUrl, protocol: profile.protocol, upstreamProtocol: profile.upstreamProtocol, schemaProfile: profile.schemaProfile, proxyMode: profile.proxyMode ?? "inherit", authScheme: profile.authScheme, credentialMode: profile.credentialMode, credentialEnv: profile.credentialEnv, credentialValue: undefined, allowInsecureHttp: profile.insecureHttp, headers: Object.entries(profile.extraHeaders).map(([name, value]) => ({ name, source: "environment", value })), models: profile.models }; }
function candidateModel(value: ProviderModelCandidate): ProviderModelDefinition { return { id: value.localId, model: value.remoteId, name: value.name, description: value.description, contextWindow: value.contextWindow, inferenceIdleTimeoutSeconds: DEFAULT_PROVIDER_INFERENCE_IDLE_TIMEOUT_SECONDS, reasoningEfforts: providerReasoningEfforts(value.remoteId, value.reasoningEfforts) }; }
...
function protocolUpstreamLabel(protocol: ProviderProtocol): string { return protocol === "responses" ? "OpenAI Responses" : protocol === "messages" ? "Anthropic Messages" : "OpenAI Chat"; }
