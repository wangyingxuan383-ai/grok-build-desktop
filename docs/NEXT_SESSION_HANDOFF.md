...
## 2026-09-18 能力实施入口（未发布）

09-19 已按审查修正：保留绑定任务工作区编辑并接入会话迁移、间隔锚点保持、重复日历触发、等待不占槽、MCP fresh/reuse 和会话级证据。定向 58 项及任务中心 DOM、类型检查、未注册 Windows XML 解析通过；未重跑全量。先读实施记录顶部的 09-19 节，旧“Worker 重登记”说明已被替代。

先读 [Desktop 能力实施记录](DESKTOP_CAPABILITIES_IMPLEMENTATION_2026-09-18.md)。工作区包含离线实现与新增测试，保留原交接补记。后续先核验实际选中 CLI 的 hook/子智能体合同，不把离线测试或旧电脑实测当成本机模型验收。新电脑此前读取的默认受管 CLI 为 1.0.30；下方 1.0.25 等记录属于旧电脑历史。

## 2026-09-17 跨电脑交接入口

先读 `SESSION_TRANSFER_2026-09-17.md`。交接包包含Git bundle、源码与本地文档补记、未发布下载器快照、可选实机脚本和校验清单；不携带账号、代理私有设置或会话数据。本轮只交接，不发布或升级。

## 2026-09-11 v0.9.5 发布授权与当前状态

- 用户明确要求先打包上传之前内容，发布范围为已完成的更新恢复、可选探针隔离、代理检查和5分钟单次安装预算；版本0.9.5。此前“未授权发布”仅为历史记录。
- 新分段下载器代码暂存于忽略目录 `out/audit/deferred-downloader`，本版不包含；它的独立真实试验已成功安装1.0.25并恢复/删除测试会话，但尚缺自动化回归。
- 当前本机CLI实际为1.0.25 (f7e67d6988e2)，不是下方历史记录的1.0.3；桌面安装版未替换。
- PR #51全部门禁通过并合并至 `fefac4375cb9e063787cbcfa22228a71f7bdc429`，标签v0.9.5已推送；正式工作流34601626752的build和download-verify-and-publish均成功；五项资产已上传、下载回验通过，v0.9.5已作为非预发布Latest公开。发布页：https://github.com/wangyingxuan383-ai/grok-build-desktop/releases/tag/v0.9.5。用户三份 `_tmp_*` 不读写、不提交。

## 优先读：五分钟约束

- 用户不接受继续30分钟盲等。当前源码单次安装预算改为5分钟，锁重试共享；超时保留脱敏输出/修复exit竞态。41项聚焦测试和TypeScript通过，未发布。
- 90秒真实短诊断后CLI仍1.0.3。Clash日志确认GLOBAL出口到Grok服务连接超时，但没有x.ai下载自身失败日志，不宣称网络根因完全定位。详细边界见`CLI_PUBLIC_UPDATE_2026-09-10.md`顶部。

## 优先读：2026-09-10 公网路径跟进

- 详见 `CLI_PUBLIC_UPDATE_2026-09-10.md`。无缓存真实更新固定stable1.0.25，自行到达30分钟超时；不是人工中止。CLI仍1.0.3、哈希不变，同一会话恢复、状态idle；官方删除重试成功。
- 旧源码假运行时精确复现“实际版本不是固定目标”：回滚后再验证历史1.0.24，当前1.0.0自然不符。异机首次失败的原始顺序仍待日志。
- 修复版AppReleaseService在真实Electron中通过应用代理成功取得Latest0.9.4。未新增产品代码、安装或发布；公网安装成功仍未证明。本次下载临时文件清理被工具拒绝，已保留，不绕过。

## 优先读：2026-09-09 本机真实升级复现

- 用户本轮明确授权真实 CLI 升级；完成 1.0.3 → 1.0.24 实机试验。详见 `CLI_LIVE_UPGRADE_2026-09-09.md`，取代下方“本轮未升级”的历史边界。
- stable 指针和官方检查都是 1.0.24，官方 EXE 实际也为 1.0.24；不能因 Changelog 只到 1.0.13 就认定它是测试版。
- 原版更新器真实复现：新旧版非 Git 探针均失败 → 回滚后 retained 残留 → 会话门禁拒绝。修复版接管真实失败清单后恢复原会话，再次升级 1.0.24 核心门禁及同一会话恢复成功。
- 公网单连接很慢，先人工停止未完成下载；后续安装使用已校验官方原文件的进程级回环缓存，不冒充默认网络路径已通过。最初“实际版本不是固定目标”仍未复现。
- 桌面安装版未替换，未推送/发布；测试凭据留在独立目录，不写正式 AppData。最终 CLI 恢复到测试前版本的结果见实机报告末尾。

## 优先读：2026-09-09 另一台电脑升级失败补修（Unreleased）

- 正式基线已是 0.9.4；用户三张截图暴露升级/回滚可选探针故障、策略恢复死路、应用代理未接入 Release 检查。源码修复及证据见 `UPDATER_RECOVERY_2026-09-09.md`。
- Git/Info/Usage/rename 的可选异常不再阻断核心门禁；恢复中的“重新验证”验证当前实际版本。可重新选择策略升级，连续失败保留最初动态回滚点。旧版在盘不重复下载；未验证状态不放行、不重发。
- 控件不再等待联网刷新解锁；应用 Release 专用网络分区接入设置代理，403 按证据分类，提供发布页入口；失败不缓存六小时。
- 最初“实际版本不是固定目标”的具体机器原因尚未确定。已增加目标/实际/路径诊断和官方手动安装 no-op 识别，不能据此宣称那台电脑已能升级。
- 没有升级本机 CLI、安装/打包、提交/推送或 Release。本轮发布需用户新授权；不要沿用历史 0.9.4 发布授权。三份用户 `_tmp_*` 未读写。
...
- 用户反馈：自动批准仍弹权限；Plan 也不该再问，并不要再限制编辑；设置默认模型是 4.5 且列表为空；月额度接口有变；官方 CLI 可能到 1.0.3。
- 源码已改为 Agent 询问、Auto/Plan 全自动批准（含写入），Resume 保留 Auto。设置模型/思考档复用会话 ACP 目录。月额度解析更宽松。
- 官方 GitHub 源码 crate 为 **1.0.1**，公开 Changelog 仍是 1.0.0。未证实稳定频道 1.0.3，禁止对本机正在使用的 CLI 擅自升级。
- 未覆盖安装当前对话窗口。正式资产等 `v0.8.3` 标签工作流。
...
- 再审后的完整离线门禁为 101 个测试文件/734 项通过，6 个 live 文件/9 项按设计跳过；TypeScript、资源/生产构建、367 文件公开扫描、Renderer 分块、Native Host、零漏洞依赖审计、diff check 与当前生产构建 UI 通过。无推理 CLI 探针确认 Session Rename 在 stable 1.0.0 中为 method-not-found。
...
- 最终源码门禁已完成：101 个测试文件/734 项通过，6 个 live 文件/9 项按设计跳过；TypeScript、资源/生产构建、367 文件公开扫描、分块、Native/Fuses、`npm audit`（0 漏洞）、打包版与安装版 UI 均通过。
...
本次在 `codex/v0.7.0-audit-hardening` 的既有 0.7.0 候选之上继续实现了推荐顺序中尚未落地的安全前向面：

- 主进程按运行时能力证据接入官方 `/btw` 旁路提问、标准 `session/list` 以及 `x.ai/session/info`、`x.ai/session/usage`；参数使用官方 `session_id`/`question` 形态，未声明能力时返回明确 unsupported，不按版本号猜测。
- Preload/IPC Schema、App Composer 和右侧工具栏已接通；Composer 只有在当前 CLI 命令证据包含 `btw` 时显示“旁路提问”，右侧“会话信息”只显示 CLI 实际返回的模型、模式、思考档位和 Token。
- Mermaid 计划块增加复制源码、复制图像和打开图像按钮；渲染失败或尚未完成时不会启用图像操作。
- 受影响测试 59 项通过；完整离线套件为 **94 个测试文件、676 项通过、9 项按设计跳过**；TypeScript、生产构建通过。当前新增源码尚未重新打包安装，因此后续必须先运行资源/生产门禁、重新生成唯一 0.7.0 Setup/Portable 资产，再做安装版冷启动检查。
- 0.2.119/0.2.120 stable 通道尚未由本机下发；上述面向使用真实运行时声明的能力门控和脱敏 Fixture 验证，不等于这些版本的实机验收。MCP/插件状态、官方 Git UI 与 `doctor fix` 预览仍延期。

下一步严格顺序：

1. 运行 `git diff --check`、`npm run check:public`、`npm run check:chunks`、`npm run build:resources`、完整 `npm test`、`npm run typecheck` 和 `npm run build`。
2. 重新执行一次 0.7.0 正式打包与 release 资产生成；保留旧 0.6.x/0.7.0 资产，不推送 GitHub、不创建 Release。
3. per-user 安装新候选，冷启动验证 About、诊断、进程版本、ASAR/Fuses、桌面和开始菜单快捷方式；随后提交本地分支基线，等待用户桌面验收。
...
- 当前证据：8 个聚焦测试文件、121 项通过；随后完整门禁 94 个测试文件、676 项通过（9 项按设计跳过），TypeScript、生产/资源构建、公开扫描和 Renderer 分块门禁通过。首次候选打包暴露离线媒体夹具 PNG 不被 Electron `nativeImage` 接受，已替换为真实 RGBA PNG；同轮还将 Windows `EPERM/EBUSY` 短暂锁竞争纳入重试。最新 Setup/Portable 已从同一构建重新生成，打包版 UI、Fuses、Task Scheduler、中文/空格 Portable 冷启动均通过；未推送、未创建 Release。
...
> **验证证据：** 最新候选完整门禁通过 94 个测试文件、676 项离线测试；6 个 live 文件、9 项按设计跳过。TypeScript、生产/资源构建、341 文件公开扫描、Native/Computer Host、24/24 Computer Use、Electron Fuses、Renderer 分块、0.7.0 打包 UI、Task Scheduler、中文/空格 Portable 冷启动及 `npm audit`（0 漏洞）通过。真实 CLI 0.2.118 最小只读 Plan 在 19.28 秒内完成，无权限卡、无工作区写入。真实自定义 Provider、双模型并行及既有长会话 Stop 仍是用户验收边界。

> **0.7.0 本地产物：** Setup SHA-256 `04f6894dea09c900f9dbae1c38d3ca6316bfd07b52da35cb97a56b57c6109a7c`；Portable SHA-256 `c1f2e74685e7f03db937141efdf6fe70c571bb328525249791055128c81a6c4f`；SBOM SHA-256 `a7a47e9a7fdb668f165f9f3813a4bd3807d99ba119155715ab0c129d36790561`；许可证报告 SHA-256 `382a29a84ec5dcf10c5e4532b34ae419aad97674b4fc36ea2a1ef504eef252c5`。桌面和开始菜单快捷方式均指向 `%LOCALAPPDATA%\Programs\Grok Build Desktop\Grok Build Desktop.exe`，ASAR 与 Fuses 已验证。
