...
- [ ] Renderer 失败诊断面板与错误卡上的入口（现在接旧诊断中心只会重现「没什么用」的观感，需与新面板一起上）。
- [ ] 真实上游触发一次失败并端到端验证分类与建议。
...
- [ ] **需用户决策**：`setWorkbenchView("files")` 全仓库无人调用，`FileWorkbench` 是渲染在不可达状态上的死 UI。0.6.5 刻意把「文件」移出侧栏且有探针断言其不得回归，因此未擅自改回。
- [ ] 两个空的设置分类。
- 复审提出的「焦点指示器被抑制」经核查不成立：样式表中不存在 `outline: none`，未做改动。

#### D5. 其余 0.6.6

- [ ] 新增真实 `TurnFileChange` 快照并完成 Git/非 Git Last turn 与 Session changes；非 Git 不显示暂存/提交/分支假功能。
- [ ] 最近文件按项目、受信任外部和不可用分组；补齐 Worktree 根目录、历史无基线、二进制/重命名/删除和可靠跳转。
- [ ] 持久化精确 `TurnUsage`，实现逐回合耗时/Token、24h/今日/7天/30天/月度统计和 53 周活动图。
- [ ] 完成 Chat/Responses/Anthropic/Gemini 请求、工具调用、流式输出与错误转换，以及模型别名/排除/Payload 规则。
- [ ] 完整离线套件、正式打包、安装验收和 `v0.6.6` GitHub Release。
