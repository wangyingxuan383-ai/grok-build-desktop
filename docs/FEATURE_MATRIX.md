...
| Hosted Runner UI probe | Fixed, v0.5.6 retry pending | Local packaging retains one long GPU-backed 3840×2160 stress flow. GitHub uses software rendering at 1920×1080, then verifies task, extension and media overlays in separate fresh Renderer processes. Lazy Suspense replacement now re-establishes modal focus. v0.5.0–v0.5.5 all stopped before Draft creation and published no assets |
