...
- `x.ai/auto-topup-rule`, rolling model limits, PAYG balance, prepaid balance and subscription tier are independent evidence. Missing `creditUsagePercent` remains unknown instead of being estimated.
...
- Closing/deleting an attached session lists active background tasks and sends `x.ai/task/kill` with `source=teardown`, then cancels running child agents before releasing the process.
