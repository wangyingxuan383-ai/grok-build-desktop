...
- Every Desktop-managed ACP, media, automation and probe process receives `--no-auto-update`. Startup/24-hour checks can report a newer stable target, but only an explicit **Update and verify** action may mutate the CLI.
- Billing follows the 1.0.3 credits contract. An attached official session is asked through `x.ai/billing`; otherwise Desktop requests `billing?format=credits`. `currentPeriod.type` owns the weekly/monthly label. Deprecated `monthly_limit` / `used` values are not treated as a subscription allowance.
- `x.ai/auto-topup-rule`, rolling model limits, PAYG balance, prepaid balance and subscription tier are independent evidence. In the proto3 credits contract, a valid `currentPeriod` with an omitted `creditUsagePercent` is the wire representation of 0 and is rendered as `0%`; an absent/invalid period remains unknown and is never estimated.
- CLI 1.0.1+ Rewind is conversation-only. Desktop never asks the official endpoint to restore files; Git Review or `TurnFileChangeJournal` provides a separate preview and confirmation surface.
- Tool read-only status is accepted from explicit ACP fields or nested `x.ai/tool` metadata only. It is used for display/audit; permission behavior still follows the CLI options and the user's Agent/Auto/Plan policy.
- Model identity is runtime-catalog driven. `x.ai/models/update` refreshes the list, a temporarily absent current model remains visible as current/unavailable, and provider-local identity is not replaced by an upstream alias.
- A new-task composer may run an initialize-only ACP probe to populate the model directory and effort metadata before `session/new`; the probe is disposed immediately and cannot create an empty Grok history entry. Live 1.0.3 evidence on 2026-08-15 returned Grok 4.6, Grok 4.5 and additional runtime-managed models with model-specific effort lists.
- A redacted live `billing?format=credits` probe on 2026-08-15 returned `weekly`. This is the sole current subscription period supplied by that account; absence of a second monthly subscription period is not a Desktop query failure.
- Live 1.0.3 command/tool evidence advertises `bundled:imagine` plus `image_gen`, `image_edit`, `image_to_video` and `reference_to_video`. Desktop uses this evidence rather than a hardcoded `/imagine` command. The probe does not perform a paid generation.
- `grok du --json` is available to Diagnostics. `grok trace` is an explicit high-sensitivity export because it may contain conversation and tool content; it is not included in normal redacted support bundles.
- Closing/deleting an attached session, application shutdown, idle reap and session-cap eviction list active background tasks and send `x.ai/task/kill` with `source=teardown`, then cancel running child agents before releasing the process. Reap/cap skip disposal when teardown fails.
- Official source snapshot: repository commit `eb267fe...`, `SOURCE_REV e6a67a...`, crate 1.0.3. Runtime declaration and observed evidence continue to outrank version hints.
