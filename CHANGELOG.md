...
## 0.9.6 - 2026-09-19（发布准备中）

- 用户授权发布此前 Desktop 能力更新和审查修正，版本统一为 0.9.6。
- 中文发布说明与未完成验收边界见 [v0.9.6](docs/releases/v0.9.6.md)。本地打包、远程上传与回验状态完成后补记；下方 Unreleased 为实施阶段历史记录。

## Unreleased - 2026-09-19 审查后修正

- 保留绑定任务工作区编辑，下次执行迁移同一会话；固定执行字段提前展示，减少保存失败。
- 修复普通编辑重置调度锚点；日历改用重复触发器和持久到期游标，不依赖 Worker 重登记。
- 等待当前会话不占全局执行槽，统一无活动超时；MCP 支持独立任务 fresh/reuse，能力证据按会话隔离。
- 58 项定向测试、类型检查、任务中心定向 DOM 与 Windows 未注册 XML 解析通过；未重跑全量、未注册真实任务或调用模型。真实 CLI 验收仍待进行。

## Unreleased - 2026-09-18 Desktop 能力闭环

- 修复持久任务完成覆盖编辑/暂停/删除，分离 revision、运行映射和删除标记，增加终态事务与崩溃恢复。
- 接入 Desktop MCP 和持久任务 Skill，增加当前会话转交、跨进程会话独占、时区/锚点调度；fresh 保留历史。
- Computer 增加服务端开关、持久确认桥接、Windows 桌面 mutex 和主会话调用证明；统一自然语言 Skill 与 Auto/Agent/Plan。
- 保留 Agent 未知字段和 MCP 继承配置，修正子智能体取消身份；不再把历史验收显示为当前成功。
- 本机 CLI hook、真实模型及 Windows Scheduler 验收待单独进行。详见 [实施记录](docs/DESKTOP_CAPABILITIES_IMPLEMENTATION_2026-09-18.md)。未发布。

## 2026-09-17 交接记录（非版本发布）

- 新增 `docs/SESSION_TRANSFER_2026-09-17.md`，整理v0.9.5发布、实测边界及未发布下载器的后续任务，生成跨电脑开发交接包。未修改产品代码或发布新版本。

## 0.9.5 - 2026-09-11

发布完成：正式工作流34601626752打包、上传及下载回验全部成功，Setup/Portable与校验文件已作为Latest公开。
...
- 超时保留脱敏的最后8KiB子进程输出，修复终止时exit事件覆盖超时原因；聚焦41项测试和TypeScript通过。此变更限制等待，不宣称修复公网下载速度。

### 2026-09-10 实测补记

- 无缓存公网更新stable1.0.25完整触发30分钟超时，旧版及哈希不变、同一会话恢复、状态解锁；公网安装成功仍未通过。CLI保持1.0.3。
- 旧版回滚后验证历史目标的模拟精确复现截图版本不符错误；修复版应用代理在真实Electron中成功查询GitHub Latest0.9.4。见 `docs/CLI_PUBLIC_UPDATE_2026-09-10.md`。本轮未新增产品代码或打包发布。

### 2026-09-09 更新失败恢复补修

- 本轮后续获用户授权完成真实 1.0.3 → stable 1.0.24 安装、ACP 核心门禁和同一会话恢复；原版更新器的非 Git 探针/回滚隔离故障链已实机复现。公网单连接下载慢，安装阶段使用经校验的官方原文件缓存；不宣称默认网络下载已修复。详见 `docs/CLI_LIVE_UPGRADE_2026-09-09.md`。
- 修复非 Git 临时目录中的可选 Git/Session Info/Usage/重命名检查异常被误判为核心 ACP 验证失败；核心 initialize/new/resume/close/delete 仍必须成功。
- 恢复状态下可重新选择策略并预览新的 stable 目标，持续保存最初升级前的动态回滚点和待恢复会话。重新验证改为核验当前实际 CLI；已经在回滚版本时不重复下载。
- 修复更新结束后联网刷新阻塞升级策略和恢复按钮；迟到刷新不能覆盖下一次操作。无法通过回滚验证时保留恢复清单，不再尝试启动必然被隔离的会话。
- 应用 Release 检查接入应用代理，区分可识别限流与未知 403，提供官方发布页入口，失败不再被缓存六小时。没有绕过代理、附加用户令牌或自动下载应用。
- 目标版本不符时显示目标/实际版本/执行路径；识别官方 CLI 对手动安装返回成功退出却没有更新的情况。截图中最初版本不符的具体成因仍待另一台电脑日志核对。
- 本节初始离线修复记录见 `docs/UPDATER_RECOVERY_2026-09-09.md`；后续获授权的本机升级/回滚实测见上方实机报告，最终已恢复原 CLI 1.0.3。

## 0.9.4 - 2026-09-09

用户已授权将本节累计内容正式发布。中文简洁版和详细版见 `docs/releases/v0.9.4.md`；CLI 实机证据仍为 1.0.3，不将离线兼容测试解释为已升级到 1.0.13。以下日期小节保留开发阶段当时的验证状态。
...
### 2026-09-08 — 累计缺陷修复与单次 CLI 分级升级

- 修复提交完成前自动保存删除长附件、失败快照覆盖新草稿及重试后旧草稿复活；提交引用持久化至现有 UI 存储，崩溃重启保留恢复内容但不自动重发。
- 新任务统一草稿恢复所有权；迟到队列、插话、BTW、Stop 回执与提示不污染其他会话。修复菜单 toggle 回灌、方向键焦点和 Toast 复制失败/旧状态残留。
- CLI 更新新增标准、尝试新版、强制保留未验证新版三种**单次**策略，仍固定官方 stable 目标、明确确认并执行核心 ACP 验证。未验证保留有独立恢复入口与实时会话隔离，不假装验证成功。
- 更新错误、日志错误、回滚和恢复分别结算；失败主因不会被日志覆盖，手动回滚不再依赖 stable 检查成功。未知版本授权绑定二进制路径/版本/哈希；验证期间被替换则拒绝签发。
- 修复主/可选/认证探针异常清理；认证不再直接删除官方会话目录。MCP 表单保留数值、长度和枚举约束，复杂/无效约束明确不支持，修复 IPv6 回环识别。
- About/Settings 复用同一更新策略控件，显示检查与下载/验证/回滚/恢复阶段。独立 MCP 卡片复用原行为以加入真实 DOM 回归，未重做主题。
- 完整离线结果：917 通过、9 live 跳过；TypeScript、生产构建与分块门禁通过。没有升级 CLI、打包、安装、推送或发布；安装版和真实更新验收仍需单独执行。

以下 09-07 为修复前审核记录，当前源码状态以上述补记和审核报告为准。

### Audit status — 2026-09-07

- 对比正式 v0.9.3 的复核发现仍待修复的草稿覆盖、长附件失败恢复、CLI 未知版本门禁和更新异常结算问题；详见 `docs/POST_V093_REVIEW_2026-09-07.md`。本轮只审核与清理已确认测试会话，未宣称修复或批准发布。

### Added

- 高频 Renderer 首轮收口：新增独立 App Shell、Dialog Host、会话行、全局错误 Toast 和会话级 Submission Controller；保留既有虚拟列表、草稿、队列、主题及沙箱协议，不建立第二套状态系统。
- 侧栏会话行改为完整键盘可操作的主按钮，置顶、归档、导出、重命名和删除收进统一菜单；Composer 运行态改为“加入队列”主动作、次级发送菜单和固定 Stop；About 中应用更新与 CLI 更新改为两个独立状态区。
- 离线 UI 探针在 1280×720@125%、1440×810@150% 和 1920×1080@200% 下生成确定性截图到 `out/ui-snapshots`，同时继续检查 Composer 与右栏边界。
- 将 Grok Build CLI 离线审计范围从 `1.0.5` 扩展到 stable `1.0.13`，补齐 `1.0.6–1.0.13` 的脱敏 initialize/事件 Fixture、稳定 ACP 客户端身份和图片感知读取策略。完整审核见 `docs/CLI_1_0_13_AUDIT_2026-09-04.md`。
- 新增 MCP 1.0.8+ form/URL elicitation：MCP 服务器可在当前回合请求有限字段输入或系统浏览器授权；表单 Schema、URL、字段和响应大小均在主进程校验，Stop/退出会取消未决请求。
- 更新兼容门禁折中策略：已审计 `1.0.0–1.0.13` 走离线合同；未来同一 `1.0.x` 只允许用户触发固定目标更新并通过本机 live ACP 门禁；`1.1+` 继续失败关闭。

### Fixed

- 修复持久新任务草稿已经是当前逻辑目标时，再点击草稿行不会触发 key 变化、因而显示空 Composer 的问题；现在会显式恢复正文、能力、附件和任务配置。
- Plan、权限、问题和 MCP 输入统一请求卡正文/状态/操作层级；问题继续支持自由文字，Plan 继续支持 Agent/自动批准策略。已完成过程默认折叠，正在运行或含错误的过程保持展开。
- 长附件、请求表单、脚本、错误和全局 Toast 增加独立边界；应用级错误可复制诊断，且不再以无限高度遮挡 Composer。
- 修复“转为附件”后的长文本在发送前被草稿清理删除：Renderer 不再先删草稿目录，主进程先把附件复制到会话缓存，再消费草稿记录；新任务草稿迁移会采用主进程返回的新路径，失败仍可恢复原正文和附件。
- 修复应用重启后，文本草稿因进程内文件签发记录丢失而无法再次发送。主进程现在按会话绑定的应用缓存目录重新校验，拒绝跨会话草稿路径，同时允许合法的持久草稿恢复。
- Markdown 正文增加统一有界容器；账号串、哈希、Base64 和其他无断点长文本会在消息卡内换行，代码块与表格保留自己的横向滚动，不再伸出对话框。
- Plan 批准卡新增“Agent 询问 / 自动批准”执行策略。进入 Plan 前为 Auto 时默认保持 Auto；用户也可在批准前显式切换。修复批准 Auto 后迟到的 Plan 模式回放又把策略降为 Agent 的竞态。
- CLI 更新明确显示使用应用代理、环境代理或系统网络/直连。更新等待上限从 6 分钟提高到 30 分钟，并防止重复点击；目标下载失败但二进制仍是原版本时不再重复下载一次原版本作为“回滚”。目标失败、回滚失败和会话恢复失败会分别保留，不再用清理错误覆盖首要原因。
- 修复关闭应用可能中断 CLI 二进制替换/回滚事务：更新完成前退出会被阻止。恢复部分会话失败现在作为 warning 附在主要更新回执上，不再把成功更新或成功回滚错误地改写成总失败。
- 修复长附件预处理期间的草稿世代竞态：提交携带 `submissionId`，主进程只分离匹配世代的草稿行并保护正在复制的旧文件；用户同期输入或转出的下一条附件不会被旧提交删除、覆盖或停止自动保存。
- 修复未知未来 1.0.x 被用户在应用外替换后仍可直接启动：没有当前 Desktop 数据目录中的通过回执时，会在创建 ACP Session 前失败关闭。
- MCP URL 完成通知同时核对会话、elicitation ID 和可选服务器名，避免同 ID 的其他服务器误关当前卡片；临时授权 URL/表单内容不写入 ConversationProjection。
- 上游跟踪快照更新到 stable 1.0.13、仓库 `72a61251...` 和 `SOURCE_REV a549186d...`，并停止对同一个“stable 已审核但尚未 live 更新”状态每周重复评论。
- 修复依赖审计公告：锁定 `@xmldom/xmldom 0.9.12`、`fast-uri 4.1.4` 和 `qs 6.16.0`，不改变 Electron 沙箱与主进程边界。

### Verification

- 高频 UI 新增组件/布局回归均通过；完整离线套件为 123 个测试文件中 117 通过、6 个 live 文件按设计跳过，874 项通过、9 项 live 跳过。当前版本 CDP UI 验收通过 Plan/权限/Stop/多会话隔离/草稿恢复及三档缩放布局。
- CLI 1.0.13 本轮只做只读 stable/源码/Changelog 审计，没有执行 CLI 更新；本机/live 基线仍为 1.0.3。

- 聚焦草稿/IPC/ACP/更新事务等 7 个测试文件、193 项通过，覆盖草稿世代与文件保护、Plan Auto 策略和迟到回放、MCP elicitation、CLI 代理路径及更新/回滚终态。
- 完整离线套件：119 个测试文件中 113 通过、6 个 live 文件按设计跳过；867 项通过、9 项 live 跳过。TypeScript、生产构建、Renderer 分块预算、`npm audit --audit-level=high`（0 漏洞）、上游快照校验和 `git diff --check` 通过。
- 排除三份用户自有 `_tmp_*` 文件的隔离副本通过公开安全扫描（434 个文本文件）。物理工作树扫描会按设计拦截这些含本机路径的用户文件，因此不将其误报为仓库缺陷。
- 2026-09-06 再次只读确认本机 CLI 1.0.3、stable 1.0.13、内部安装器且 CLI 自动更新关闭。本轮没有执行 CLI 升级、打包、安装、推送或 Release。
...
- 新任务草稿会通过一次不创建会话的 ACP `initialize` 读取官方模型目录，并合并已启用的自定义 Provider 模型；刷新后可直接选择 Grok 4.6、未来官方模型及其真实思考档位。
...
- 会话删除、关闭、应用退出、空闲回收或并发上限淘汰前会以 1.0.3 `teardown` 语义停止所属后台任务和子 Agent；回收清理失败时保留会话，避免直接杀掉仍有后台工作的进程。
...
- 修复输入正文后删除新任务草稿时，250ms 自动保存与附件缓存占用可能让删除报错或把草稿重新创建的问题；删除现在先停止自动保存并清空本地状态，附件缓存由重试和启动清理兜底。
- 修复没有先打开正式会话时新任务模型选择器只剩 Grok 4.5、推理档位使用硬编码全集的问题；档位严格来自所选模型能力，自定义模型继续保留 Provider 身份。
- 额度页明确说明 credits 接口只返回一个当前订阅周期：账号返回周周期时不会伪造另一份月订阅额度；PAYG 月上限仅在官方返回有效正数时单独显示。
- 额度实机探针不再让 PowerShell JSON 解析错误回显 OAuth 文件原文，并对诊断中的令牌形态做二次脱敏和长度限制。
...
- Agent 仍会询问工具权限。自动批准和 Plan 现在批准全部工具，包括写入和命令，不再弹出权限卡，也不再把 Plan 当成只读沙箱。
- 恢复会话时保留 Desktop 的「自动批准」覆盖层。官方 CLI 只报告 plan/default，以前会把 Auto 静默打回 Agent，导致界面显示自动批准却继续要权限。
- 设置里的默认模型和推理强度改用与会话输入框相同的 ACP 模型目录和该模型声明的思考档位；不再手写完整档位列表。Grok 4.5 的 `xhigh` 只要 CLI 握手声明就会出现。
- 模式文案改为：Agent（询问）、Plan（自动批准，可编辑）、自动批准。Plan 结束确认卡仍保留。

### Fixed

- 月额度解析兼容百分比、嵌套 `subscription`/`limits` 以及 `monthlyCredits`/`monthlyUsed` 等新字段，避免官方账单接口改形后整栏空白。
...
- 已生成并 per-user 安装 0.7.3；安装版主进程/About/诊断、ASAR、Fuses、桌面/开始菜单快捷方式均验证。Setup SHA-256 `28408514697e75b122a2f879083b9fea9e022e68f2f98e987e19173f7e5948ac`；Portable `8d5740b12ec5674d2cbb110afb13dc8b38c5ab1e92f6e2a15ab03feab0dcc277`；SBOM `96c5307ea15185f11194d0150a175ed5c16a91bb352faeef28eb8229b44abc1f`；许可证报告 `668db4dc469c6d4dd9616429ef14daa1032e93ffc6fece8355863b85ecbe21dc`。用户验收前不推送、不创建 GitHub Release。
