...
- The first PR Windows dependency gate exposed two optional `@emnapi` packages absent from the lockfile under npm 11 strict `npm ci`. Regenerating the lock metadata added only those optional transitive entries; `npm ci --dry-run` and the production-only audit then passed without rebuilding the already verified candidate.
